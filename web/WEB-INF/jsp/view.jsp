...
<%@ page import="com.topjava.webapp.model.OrganizationSection" %>
<%@ page import="com.topjava.webapp.util.HtmlUtil" %>
...
<section>
    <h1>${resume.fullName}&nbsp;<a href="resume?uuid=${resume.uuid}&action=edit"><img src="resources/pencil.png"></a></h1>
    <p>
                        ...
                        <tr>
                            <td colspan="2">
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <h3><a href="${org.link.url}" target="_blank">${org.link.title}</a></h3>
                                    </c:otherwise>
                                </c:choose>
                            </td>
                        </tr>
                        <c:forEach var="position" items="${org.positions}">
                            <jsp:useBean id="position" type="com.topjava.webapp.model.Organization.Experience"/>
                            <tr>
                                <td width="15%" style="vertical-align: top"><%=HtmlUtil.formatDates(position)%>
                                </td>
                                <td><b>${position.title}</b><br>${position.text}</td>
                            </tr>
                        </c:forEach>
                    </c:forEach>
                </c:when>
